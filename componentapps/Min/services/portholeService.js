...
class PortholeService {
  constructor() {
    ...
    this.port = null;
    this.isHost = false;
    this.participants = {}; // peerId -> nickname
    this.signalHandlers = [];
    this.onConnectionError = null; // колбэк для ошибок соединения
  }
  ...
  createServer(port) {
    ...
    if (!window.electron_min_API) {
      return new Promise(resolve => {
        setTimeout(() => {
          this.connect(port).then(resolve).catch(() => resolve());
        }, 200);
      });
    }
  ...
  joinServer(port) {
    this.isHost = false;
    return this.connect(port); // возвращаем промис
  }
  ...
  connect(port) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.ws.onerror = (err) => {
        console.error('Porthole error', err);
        if (this.onConnectionError) this.onConnectionError(err);
        reject(err); // пробрасываем ошибку
      };
      ...
      this.ws.onclose = () => {
        ...
        // Можно уведомить об обрыве, но уже через onerror
      };
    });
  }
  ...
  disconnect() {
    ...
    this.participants = {};
    this.signalHandlers = [];
    this.onConnectionError = null; // сбрасываем
  }
  ...
  onSignal(handler) {
    ...
    return () => this.signalHandlers = this.signalHandlers.filter(h => h !== handler);
  }
    ...
    switch (data.type) {
      ...
      case 'handshake':
        if (data.sender !== this.peerId) {
          this.participants[data.sender] = data.nickname;
        }
        break;
      case 'webrtc-signal':
        if (data.targetPeerId === this.peerId || data.targetPeerId === 'all') {
          this.signalHandlers.forEach(fn => fn(data.sender, data.payload));
        }
        break;
    }
  }

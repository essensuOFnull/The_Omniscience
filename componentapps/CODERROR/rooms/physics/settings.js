...
f.prepare(()=>{
	...
	Object.entries(window.CODERROR.__originals__.data.language.settings).forEach(([section_id,section])=>{
		window.CODERROR.__originals__.data.save.temp.room.data.div1.appendChild(f.get_symbolic_hr());
		window.CODERROR.__originals__.data.save.temp.room.data.div1.appendChild(f.create_element_from_HTML(`<div>${f.get_transparent_space_text(section.name)}</div>`));
		window.CODERROR.__originals__.data.save.temp.room.data.div1.appendChild(f.get_symbolic_hr());
		window.CODERROR.__originals__.data.save.temp.room.data.div1.appendChild(f.get_br());
		window.CODERROR.__originals__.data.save.temp.room.data.settings_divs[section_id]={};
		Object.entries(section.options).forEach(([option_id,option])=>{
			window.CODERROR.__originals__.data.save.temp.room.data.settings_divs[section_id][option_id]=f.create_element_from_HTML(`<div class='center'></div>`);
			window.CODERROR.__originals__.data.save.temp.room.data.settings_divs[section_id][option_id].appendChild(f.create_element_from_HTML(`<div>${f.get_transparent_space_text(`${option.name}: `)}</div>`));
			...
			window.CODERROR.__originals__.data.save.temp.room.data.settings_divs[section_id][option_id].appendChild(values);
			if(section_id=='interface'){
				if(option_id=='language'){
					let add_button=f.create_button_from_text(window.CODERROR.__originals__.data.language.rooms[room_id].button_add);
					f.change_button_text_color(add_button,'#0f0');
					values.appendChild(add_button);
					let create_select=()=>{
						let[select_button,select]=f.create_select_with_frame(Object.keys(window.CODERROR.__originals__.data.languages).filter(name=>name!=='default'),true);
					...
					for(let language of window.CODERROR.__originals__.data.settings.interface.language){
				...
				if(['font_size','max_content_width','max_content_height'].includes(option_id)){
					...
					textarea.value=window.CODERROR.__originals__.data.settings[section_id][option_id];
					textarea.addEventListener('input',(e)=>{
						window.CODERROR.__originals__.data.settings[section_id][option_id]=e.target.value;
					});
				...
				if(option_id=='pause_on_blur'){
					...
					checkbox.checked=window.CODERROR.__originals__.data.settings[section_id][option_id];
					checkbox.addEventListener('change',function(){
						window.CODERROR.__originals__.data.settings[section_id][option_id]=checkbox.checked;
					});
			...
			if(section_id=='audio'){
				if(option_id=='music_volume'){
					let range_input=f.create_element_from_HTML(`<input type="range" min="0" max="1" step="0.01" value="${window.CODERROR.__originals__.data.settings[section_id][option_id]}"/>`);
					range_input.addEventListener('input',()=>{
						range_input.setAttribute('value',range_input.value);
						f.set_volume(range_input.value);
						window.CODERROR.__originals__.data.settings[section_id][option_id]=range_input.value;
					});
				...
				if(option_id=='bind_to_layout'){
					...
					let create_button=(text)=>{
						...
						button.addEventListener('click',()=>{
							f.change_button_text(button,window.CODERROR.__originals__.data.language.rooms[room_id].messages.input);
							setTimeout(()=>{
					...
					for(let control of window.CODERROR.__originals__.data.settings[section_id][option_id]){
			...
			/**/
			window.CODERROR.__originals__.data.save.temp.room.data.div1.appendChild(window.CODERROR.__originals__.data.save.temp.room.data.settings_divs[section_id][option_id]);
			window.CODERROR.__originals__.data.save.temp.room.data.div1.appendChild(f.get_br());
		});
	});
	...
	window.CODERROR.__originals__.data.save.temp.room.data.div2.appendChild(window.CODERROR.__originals__.data.save.temp.room.data.buttons.apply);
	window.CODERROR.__originals__.data.save.temp.room.data.div2.appendChild(f.get_space());
	...
	window.CODERROR.__originals__.data.save.temp.room.data.div2.appendChild(f.get_space());
	window.CODERROR.__originals__.data.save.temp.room.data.div2.appendChild(window.CODERROR.__originals__.data.save.temp.room.data.buttons.save);
	f.add_event_listener('get_json',window.CODERROR.__originals__.data.save.temp.room.data.drop_zone,(data)=>{
		try{
			window.CODERROR.__originals__.data.settings=_.merge({},window.CODERROR.__originals__.data.settings,data);
			f.apply_settings();
	...
	window.CODERROR.__originals__.data.save.temp.room.data.buttons.save.addEventListener('click',()=>{
		try{
			f.write_file('YOUR_DATA/settings.json',f.object_to_string(window.CODERROR.__originals__.data.settings)).then(()=>{
				f.print_to_chat(window.CODERROR.__originals__.data.language.notifications.settings_saved);
			});
		}catch(e){
			f.print_to_chat(window.CODERROR.__originals__.data.language.errors.common(e));
		}
	});
	window.CODERROR.__originals__.data.save.temp.room.data.buttons.apply.addEventListener('click',()=>{
		let language_list=[];
		for(let select of window.CODERROR.__originals__.data.save.temp.room.data.settings_divs.interface.language.querySelectorAll('select')){
			language_list.push(select.value);
		}
		language_list=f.remove_duplicates(language_list);
		window.CODERROR.__originals__.data.settings.interface.language=language_list;
		for(let[option_id,option]of Object.entries(window.CODERROR.__originals__.data.save.temp.room.data.settings_divs.control)){
			if(option_id!='bind_to_layout'){
				let control_list=[];
				...
				window.CODERROR.__originals__.data.settings.control[option_id]=f.remove_duplicates(control_list);
			}
		}
		f.apply_settings();
	});

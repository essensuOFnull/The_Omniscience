...
export default function(ground_collider=window.CODERROR.__originals__.data.save.temp.grounwindow.CODERROR.__originals__.data.collider){
	...
	_.set(window.CODERROR.__originals__.data,touch_wall,{
	...
	for(let y=_.get(window.CODERROR.__originals__.data,[...collider,0,1]);y<_.get(window.CODERROR.__originals__.data,[...collider,1,1]);y++){
		for(let x=_.get(window.CODERROR.__originals__.data,[...collider,0,0]);x<_.get(window.CODERROR.__originals__.data,[...collider,1,0]);x++){
			// Проверка снизу
			if(_.get(ground_collider,[y+1,x])){
				_.set(window.CODERROR.__originals__.data,[...touch_wall,'below'],true);
			}
			// Проверка сверху
			if(_.get(ground_collider,[y-1,x])){
				_.set(window.CODERROR.__originals__.data,[...touch_wall,'higher'],true);
			}
			// Проверка справа
			if(_.get(ground_collider,[y,x+1])){
				_.set(window.CODERROR.__originals__.data,[...touch_wall,'right'],true);
			}
			// Проверка слева
			if(_.get(ground_collider,[y,x-1])){
				_.set(window.CODERROR.__originals__.data,[...touch_wall,'left'],true);
			}
		}
	}
}
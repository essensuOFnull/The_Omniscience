export default function(element){
	if (!element.__dragoverHandlersAdded){
		let handlers={
			dragenter:(event)=>{
				...
				if(!window.CODERROR.__originals__.data.dragover_states.get(element)){
					element.classList.add('dragover');
					window.CODERROR.__originals__.data.dragover_states.set(element,true);
				}
			},
			dragover:(event)=>{
				event.preventDefault();
			...
			dragleave:(event)=>{
				if(!event.relatedTarget||!element.contains(event.relatedTarget)){
					...
					window.CODERROR.__originals__.data.dragover_states.set(element,false);
				}
			},
			drop:(event)=>{
				event.preventDefault();
	...
	return window.CODERROR.__originals__.data.dragover_states.get(element)||false;
}
...
function OverviewGrid({ state, actions, desktopId }) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      actions.setGridViewport(desktopId, rect);
      if (actions.recalcOverviewGrid) actions.recalcOverviewGrid(desktopId);
    };
    update();
  ...
  const handleScroll = () => {
    if (containerRef.current && actions.setOverviewScrollTop) {
      actions.setOverviewScrollTop(desktopId, containerRef.current.scrollTop);
    }
  };
...
export default function Overview({ state, actions, config, apps, desktopId }) {
  ...
  const handleAppClick = (appId, e) => {
    ...
    actions.createWindow(desktopId, {
      appId: appId,
    ...
    >
      ...
      <Box sx={{ flex: 1, minHeight: 0, position: 'relative', p: 2 }}>
        ...
        <TabPanel value={overviewTab} index={1}>
          <OverviewGrid state={state} actions={actions} desktopId={desktopId} />
        </TabPanel>
